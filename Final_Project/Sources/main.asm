...
; export symbols
            ...
            XDEF command,equal_f
            XDEF port_u, delay,repass
            ...
            XREF User_name,PW_Creation,user_chng,compare_string 
...
repass:     ds.b 1      ;retype password flag   		
...
;Show default Date and TIme			
			    ...
			    jsr     user_chng	            ;Display users name inputs on the LCD (user creates a username)
			
			    jsr     Pass_word		        ;show default password screen
			    bra     skip                    ;skip the re_enter password subroutine
			    
no_match	    jsr     re_enter_pw	
			
skip:         	jsr     PW_Creation	            ;Display users first password inputs on the LCD (User creates a password)
			
			    jsr     Pass_wordV              ;show default verify password screen
			
			    jsr     PW_Verify               ;User must re_enter same password as before to verify it
			
		    	jsr     compare_string          ;Checks if both passwords are the same
		    			    	
		    			    			
			    BRCLR	equal_f, #1,no_match    ;Branch and Retype Password (passwords entered by user didn't match)  
			    bra     yourein                 ;Branch and display a verification message (passwords entered by user match)
...
;------------DEFAULT RE_ENTER PASSWORD SHOWCASE------------------------------------
re_enter_pw:    jsr     re_enter
                ldd     #disp
                jsr     display_string
