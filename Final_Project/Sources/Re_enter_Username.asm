    XDEF    Re_Username
    XREF    command,namev,disp,cursor,enter_f
...
inc_alph:	ldx     cursor   ;load current cursor location
            ldab    namev,x   ;load alphabet letters
            cmpb	#$20
...
skip:       inc     namev,x   ;increment ascii value stored in name
			ldab	  namev,x
...
dec_alph:   ldx     cursor   ;load current cursor location
            ...
            bra     leave    ;(name was init to space '_' ($20) in main)                                    		     		
skip2:      dec     namev,x   ;increment ascii value stored in name
			ldab	namev,x   ;reload name value to check
