    XDEF    Password_start
    XREF    command,pass,disp,cursor,enter_f
...
inc_alph:	ldx   cursor   ;load current cursor location
            ldab  pass,x   ;load alphabet letters
            cmpb  #$20
...
skip:       inc   pass,x   ;increment ascii value stored in name
			ldab  pass,x
...
dec_alph:   ldx     cursor   ;load current cursor location
            ...
            bra     leave    ;(name was init to space '_' ($20) in main)                                    		     		
skip2:      dec     pass,x   ;increment ascii value stored in name
			ldab	pass,x   ;reload name value to check
